...
function platform( num_tris, points_transform )		// Arrange triangles in a fan.  This version goes all the way around a circle with them.
	{	
		shape.call(this);	
...
inherit(platform, shape);

	platform.prototype.populate = function( recipient, num_tris, points_transform, center_idx )
		{
			// if( center_idx === undefined )			// Not re-using a point?  Create one.
			// {
			// 	center_idx = recipient.vertices.push( vec3( mult_vec( points_transform, vec4( 0,0,1,1 ) ) ) ) - 1;
...
function semisphere( points_transform, max_subdivisions )		
	{	
		shape.call(this);	
...
inherit(semisphere, shape);

	semisphere.prototype.populate = function ( recipient, points_transform, max_subdivisions ) 
		{	
			var offset = recipient.vertices.length;
			recipient.vertices.push(		vec3(0.0, 0.0, -1.0) 				 );
			recipient.vertices.push(		vec3(0.0, 0.942809, 0.333333) 		 );
